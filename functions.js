...
function dniLetter( dni ) {
    ...
    if (dni==-1 || dni=="A1234567"){
        result='Invalid parameter';
        return result;
    }
    
    else {
    var calc = dni%23;  //create variable to store input number % 23
	var result = lockup[calc];  //set index to calc value and get that letter as the result
...
function findLongestWord(array_words){
    var longestword=0;
        ...
        for (i=0; i<array_words.length; i++) { //loop through array_words
...
function calculateAverage(arraylenghts) {
var calc=0;
	for (i=0; i<array_lengths.length;i++) {  //loop through array of numbers
		calc+=array_lengths[i]  //add all lengths together
	}
	return calc/array_lengths.length;  //divide all lengths added by length of the array
}
...
function charFreq(stringinput) {
    var freqCounter = {};
        ...
        for (i=0;i<stringinput.length;i++){ //for loop to go through string
